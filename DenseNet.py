...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25, result_file_name="defalut.csv"):
    with open("dense/" + result_file_name, 'w', newline="") as csvfile:
        csvfile.truncate()
        ...
        for phase in ['train', 'test']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                if phase == 'train':
                    ...
                    batch_counter += 1
                    with open("dense/" + result_file_name, "a", newline="") as file:
            ...
            if phase == 'test' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model.state_dict(), 'dense/best_model_wts.pth')
        print()
...
def evaluate_model(model, dataloaders, phase='test'):
    ...
    precision, recall, _ = precision_recall_curve([i[0] for i in y_true], [i[1] for i in y_scores])
    ...
    plt.show()
    plt.savefig(f'dense/{formatted_time} - Dense_ROC_curve.png')
    plt.close()
...
#                        num_epochs=20, result_file_name=f"{formatted_time} - densenet121_LR_0.00001.csv")
#
# plt.ioff()
# plt.show()
...
model_ft.load_state_dict(torch.load('dense/best_model_wts.pth'))
